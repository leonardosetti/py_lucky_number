...
if not SECRET_KEY:
    raise RuntimeError(
        "JWT_SECRET_KEY environment variable is required. Prevents CWE-522: no fallback secret."  # noqa: E501
    )
ALGORITHM = "HS256"
...
def validate_password_strength(password: str) -> None:
    ...
    if len(password) < 8:
        raise HTTPException(
            status_code=422, detail="A senha deve ter no mínimo 8 caracteres"
        )
    if len(password) > 128:
        raise HTTPException(
            status_code=422, detail="A senha deve ter no máximo 128 caracteres"
        )
    if not re.search(r"[A-Z]", password):
        raise HTTPException(
            status_code=422, detail="A senha deve conter pelo menos uma letra maiúscula"
        )
    if not re.search(r"[a-z]", password):
        raise HTTPException(
            status_code=422, detail="A senha deve conter pelo menos uma letra minúscula"
        )
    if not re.search(r"\d", password):
        raise HTTPException(
            status_code=422, detail="A senha deve conter pelo menos um número"
        )
...
def check_login_lockout(identifier: str) -> None:
    ...
    if record:
        if record["count"] >= MAX_LOGIN_ATTEMPTS:
            elapsed = (
                (now - record["locked_at"]).total_seconds()
                if "locked_at" in record
                else 0
            )
            if elapsed < LOGIN_LOCKOUT_MINUTES * 60:
                ...
                raise HTTPException(
                    ...
                    detail=f"Conta temporariamente bloqueada. Tente novamente em {remaining} segundos.",  # noqa: E501
                )
            else:
