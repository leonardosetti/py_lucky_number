...
from lucky_number.services.combinacao_service import save_combinacao as sv, list_combinacoes as ls, delete_combinacao as dc, toggle_favorita as tf
from lucky_number.services.promessa_service import create_promessa as cp, list_promessas as lp, delete_promessa as dp, clone_promessa as cl, share_promessa as sp
...
async def login(email: str, password: str):
    ...
    async with async_session_factory() as session:
        result = await session.execute(select(User).where(User.email == email, User.ativo))
...
async def delete_account(current_user=Depends(get_current_user)):
    ...
    async with async_session_factory() as session:
        ...
        anon_hash = hashlib.sha256(f"deleted-{user.id}-{datetime.now(UTC).isoformat()}".encode()).hexdigest()[:16]
...
async def forgot_password(identifier: str, channel: str = "email"):
    ...
    async with async_session_factory() as session:
        ...
        reset_link = f"{os.getenv('APP_URL', 'http://localhost:3000')}/redefinir-senha?token={raw_token}"
...
async def validate_reset_token(token: str):
    ...
    async with async_session_factory() as session:
        ...
        if db_token.expires_at < datetime.now(UTC):
            return {"valid": False, "error": "Este link expirou. Solicite um novo link de redefinição de senha."}
...
async def feature_audit_log(slug: str, current_user=Depends(get_current_user)):
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            select(FeatureToggleAudit).join(FeatureToggle).where(FeatureToggle.slug == slug)
        )
...
async def delete_user(user_id: str, current_user=Depends(get_current_user)):
    ...
    if str(current_user.get("id")) == user_id:
        raise HTTPException(status_code=400, detail="Você não pode excluir sua própria conta. Use DELETE /auth/account.")
    async with async_session_factory() as session:
        result = await session.execute(select(User).where(User.id == user_id))
        user = result.scalar_one_or_none()
        if not user:
        ...
        if user.role_id:
            admin_count = await session.execute(
                ...
                .join(Role).where(Role.nome == "Admin", User.ativo)
            )
...
async def delete_role(role_id: str, current_user=Depends(get_current_user)):
    ...
    async with async_session_factory() as session:
        user_count = await session.execute(
            select(func.count()).select_from(User).where(User.role_id == role_id, User.ativo)
        )
...
async def save_combinacao(jogo: str, dezenas: list[int], dezenas_por_aposta: int, current_user=Depends(get_current_user)):
    ...
    if not dezenas:
        raise HTTPException(status_code=422, detail="Lista de dezenas não pode estar vazia")
    try:
...
async def list_notifications(page: int = 1, per_page: int = 20, current_user=Depends(get_current_user)):
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            select(NotificationDelivery).where(NotificationDelivery.user_id == current_user["id"])
...
async def mark_notification_read(delivery_id: str, current_user=Depends(get_current_user)):
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            select(NotificationDelivery).where(NotificationDelivery.id == delivery_id, NotificationDelivery.user_id == current_user["id"])
        )
...
async def dashboard_summary(current_user=Depends(get_current_user)):
    ...
    async with async_session_factory() as session:
        ...
        total_bets = (await session.execute(select(func.count(Combinacao.id)))).scalar() or 0
        total_promises = (await session.execute(select(func.count(Promessa.id)))).scalar() or 0
...
async def health() -> dict:
    ...
    return {
        ...
        "redis": "connected" if redis_ok else "disconnected" if redis_ok is False else "not_checked",
