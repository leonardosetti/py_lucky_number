# Feature Specification: Mega-Sena Data Collector

**Feature Branch**: `007-megasena-data-collector`
**Created**: 2026-05-13
**Status**: Draft
**Input**: Especificação técnica para coleta de sorteios da Caixa Econômica Federal (CEF), específica para o jogo Mega-Sena.

---

## 1. Technical Overview

Coleta automatizada do histórico completo de sorteios da Mega-Sena a partir da planilha oficial da CEF, com armazenamento em JSON local (`./data/megasena.json`) e em tabela dedicada no banco PostgreSQL (`loterias_resultados_megasena`). Carga inicial integral na primeira execução; atualizações incrementais via scheduler Celery Beat com intervalo fixo de 6 horas + verificação extra 1 hora após dias de sorteio (terças, quintas e sábados, 22h). Stack assíncrona obrigatória: FastAPI + HTTPX + Polars + asyncpg + orjson + Celery/Redis.
...
| Propriedade | Valor |
|---|---|
| **URL** | `https://servicebus3.caixa.gov.br/portaldeloterias/api/resultados/download?modalidade=Mega-Sena` |
| **Formato** | Planilha eletrônica (XLSX), ordenada por concurso crescente |
| **Primeira linha** | Cabeçalho com nomes das colunas |
| **Campos por linha** | Concurso, Data do Sorteio, Bola1, Bola2, Bola3, Bola4, Bola5, Bola6, Ganhadores 6 acertos, Cidade / UF, Rateio 6 acertos, Ganhadores 5 acertos, Rateio 5 acertos, Ganhadores 4 acertos, Rateio 4 acertos, Acumulado 6 acertos, Arrecadação Total, Estimativa prêmio, Acumulado Sorteio Especial Mega da Virada, Observação |
...
- Save as `./data/megasena.json` (or `DATA_PATH`/`megasena.json`).
- Bulk insert all records into `loterias_resultados_megasena` using `asyncpg.copy_from()`.
...
- Mega-Sena draws: **Tuesdays, Thursdays and Saturdays at 21:00 BRT**.
...
1. **Given** que `./data/megasena.json` não existe, **When** o coletor executa, **Then** a planilha é baixada e convertida via Polars e orjson.
2. **Given** o JSON gerado, **When** a exportação para o banco é concluída, **Then** `loterias_resultados_megasena` contém o mesmo número de registros que o JSON.
...
### JSON: `./data/megasena.json`

```json
{
  "meta": {
    "jogo": "Megasena",
    "url_origem": "https://servicebus3.caixa.gov.br/portaldeloterias/api/resultados/download?modalidade=Mega-Sena",
  ...
  "concursos": [
    {
      "Concurso": 2800,
      "Data do Sorteio": "2026-05-09",
      "Bola1": 3, "Bola2": 12, "Bola3": 23,
      "Bola4": 34, "Bola5": 45, "Bola6": 56,
      "Ganhadores 6 acertos": 1,
      "Cidade / UF": "São Paulo/SP",
      "Rateio 6 acertos": 25000000.00,
      "Ganhadores 5 acertos": 45,
      "Rateio 5 acertos": 8500.00,
      "Ganhadores 4 acertos": 3200,
      "Rateio 4 acertos": 350.00,
      "Acumulado 6 acertos": false,
      "Arrecadação Total": 52345678.90,
      "Estimativa prêmio": 3000000.00,
      "Acumulado Sorteio Especial Mega da Virada": false,
      "Observação": ""
    }
  ]
}
...
### Database: `loterias_resultados_megasena`
...
- **FR-004**: Save JSON to `./data/megasena.json`.
...
### Monitoring
- **FR-017**: Expose `lottery_download_duration_seconds{game="megasena"}`, `lottery_new_contests_total{game="megasena"}`, `lottery_last_success_timestamp{game="megasena"}`, `lottery_errors_total{game="megasena",type="download|parse|db|json"}`.
...
- **FR-025**: JSON filename fixed as `megasena.json` (CWE-73).
...
- **FR-030**: `MegasenaCollector` inherits `BaseLotteryCollector`.
...
- **SC-001**: Initial load of ~2800 contests completes in under 90 seconds.
