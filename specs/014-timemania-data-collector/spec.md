# Feature Specification: Timemania Data Collector

**Feature Branch**: `014-timemania-data-collector`
**Created**: 2026-05-13
**Status**: Draft
**Input**: Especificação técnica para coleta de sorteios da Caixa Econômica Federal (CEF), específica para o jogo Timemania.

---

## 1. Technical Overview

Coleta automatizada do histórico completo de sorteios da Timemania a partir da planilha oficial da CEF, com armazenamento em JSON local (`./data/timemania.json`) e em tabela dedicada no banco PostgreSQL (`loterias_resultados_timemania`). Carga inicial integral na primeira execução; atualizações incrementais via scheduler Celery Beat com intervalo fixo de 6 horas + verificação extra 1 hora após dias de sorteio (terças, quintas e sábados, 22h). Stack assíncrona obrigatória: FastAPI + HTTPX + Polars + asyncpg + orjson + Celery/Redis.

**Particularidade da Timemania**: 7 números sorteados (range 1–80) + 1 "Time Coração" (time de futebol, código de 1–80). Possui faixa de premiação exclusiva para acertar o Time Coração.
...
| **Campos por linha** | Concurso, Data Sorteio, Bola1 a Bola7, Time Coração, Ganhadores 7 acertos, Cidade / UF, Rateio 7 acertos, Ganhadores 6 acertos, Rateio 6 acertos, Ganhadores 5 acertos, Rateio 5 acertos, Ganhadores 4 acertos, Rateio 4 acertos, Ganhadores 3 acertos, Rateio 3 acertos, Ganhadores Time Coração, Rateio Time Coração, Acumulado 7 acertos, Arrecadação Total, Estimativa Prêmio, Observação |
...
- Save as `./data/timemania.json` (or `DATA_PATH`/`timemania.json`).
- Bulk insert all records into `loterias_resultados_timemania` using `asyncpg.copy_from()`.
...
- Timemania draws: **Tuesdays, Thursdays and Saturdays at 21:00 BRT**.
...
1. **Given** que `./data/timemania.json` não existe, **When** o coletor executa, **Then** a planilha é baixada e convertida via Polars e orjson.
2. **Given** o JSON gerado, **When** a exportação para o banco é concluída, **Then** `loterias_resultados_timemania` contém o mesmo número de registros que o JSON.
...
### JSON: `./data/timemania.json`

```json
{
  "meta": {
    "jogo": "Timemania",
    "url_origem": "https://servicebus3.caixa.gov.br/portaldeloterias/api/resultados/download?modalidade=Timemania",
    "ultima_atualizacao": "2026-05-13T03:00:00Z",
    "ultima_verificacao": "2026-05-13T09:00:00Z",
    "total_concursos": 2200,
    "versao_formato": "1.0"
  },
  "concursos": [
    {
      ...
      "Bola1": 5, "Bola2": 12, "Bola3": 23, "Bola4": 34,
      "Bola5": 45, "Bola6": 56, "Bola7": 67,
      "Time Coração": 42,
      "Ganhadores 7 acertos": 0,
      "Cidade / UF": "",
      "Rateio 7 acertos": 0,
      "Ganhadores 6 acertos": 3,
      "Rateio 6 acertos": 45000.00,
      "Ganhadores 5 acertos": 120,
      "Rateio 5 acertos": 1800.00,
      "Ganhadores 4 acertos": 3500,
      "Rateio 4 acertos": 80.00,
      "Ganhadores 3 acertos": 45000,
      "Rateio 3 acertos": 6.00,
      "Ganhadores Time Coração": 8500,
      "Rateio Time Coração": 10.00,
      "Acumulado 7 acertos": true,
      "Arrecadação Total": 9500000.00,
      "Estimativa Prêmio": 5000000.00,
      "Observação": ""
    }
  ]
}
...
### Database: `loterias_resultados_timemania`
...
CREATE TABLE loterias_resultados_timemania (
    ...
    "Ganhadores Time Coração" INTEGER DEFAULT 0,
    "Rateio Time Coração" DECIMAL(14,2),
...
- **FR-004**: Save JSON to `./data/timemania.json`.
- **FR-005**: Bulk insert via asyncpg `copy_from()`.

### Incremental Updates
- **FR-006**: Periodic checks every 6 hours via Celery Beat.
- **FR-007**: Extra check at 22:00 BRT on draw days (Tue, Thu, Sat).
...
### Monitoring
- **FR-017**: Expose `lottery_download_duration_seconds{game="timemania"}`, `lottery_new_contests_total{game="timemania"}`, `lottery_last_success_timestamp{game="timemania"}`, `lottery_errors_total{game="timemania",type="download|parse|db|json"}`.
...
- **FR-025**: JSON filename fixed as `timemania.json` (CWE-73).
...
- **FR-030**: `TimemaniaCollector` inherits `BaseLotteryCollector`.
...
- **SC-001**: Initial load of ~2200 contests completes in under 90 seconds.
...
- **Time Coração**: Código numérico (1–80) representando um time de futebol. Armazenado como inteiro — o nome do time não está na planilha.
- **Hash**: Calculado sobre 7 bolas + time coração ordenados.
